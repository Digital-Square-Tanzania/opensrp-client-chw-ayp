...
import static org.smartregister.chw.ayp.util.Constants.STEP_FIVE;
...
public class AypJsonFormUtils extends org.smartregister.util.JsonFormUtils {
    public static final String METADATA = "metadata";

    public static Triple<Boolean, JSONObject, JSONArray> validateParameters(String jsonString) {

        JSONObject jsonForm = toJSONObject(jsonString);
        JSONArray fields = aypFormFields(jsonForm);
    ...
    public static JSONArray aypFormFields(JSONObject jsonForm) {
    ...
    protected static FormTag formTag(AllSharedPreferences allSharedPreferences) {
        ...
        formTag.appVersion = AypLibrary.getInstance().getApplicationVersion();
        formTag.databaseVersion = AypLibrary.getInstance().getDatabaseVersion();
        return formTag;
    }
    ...
    public static void tagEvent(AllSharedPreferences allSharedPreferences, Event event) {
        ...
        event.setClientApplicationVersion(AypLibrary.getInstance().getApplicationVersion());
        event.setClientDatabaseVersion(AypLibrary.getInstance().getDatabaseVersion());
    }
    ...
    public static JSONObject getFormAsJson(String formName) throws Exception {
        return FormUtils.getInstance(AypLibrary.getInstance().context().applicationContext()).getFormJson(formName);
    }
        ...
        for (Map.Entry<String, String> map : jsonStrings.entrySet()) {
            ...
            while (local_fields.length() > x) {
                try {
                    ...
                    obj.put(ayp_VISIT_GROUP, map.getKey());
                    fields_obj.add(obj);
                } catch (JSONException e) {
                    Timber.e(e);
                }
