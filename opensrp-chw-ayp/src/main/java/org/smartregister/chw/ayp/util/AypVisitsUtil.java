...
public class AypVisitsUtil extends VisitUtils {
    ...
    public static void processVisits() throws Exception {
        processVisits(AypLibrary.getInstance().visitRepository(), AypLibrary.getInstance().visitDetailsRepository());
    }
    ...
    private static void processVisits(VisitRepository visitRepository, VisitDetailsRepository visitDetailsRepository) throws Exception {
        ...
        for (Visit v : visits) {
            ...
            if (daysDiff > 1) {
                if (v.getVisitType().equalsIgnoreCase(Constants.EVENT_TYPE.AYP_FOLLOW_UP_VISIT) && getaypVisitStatus(v).equals(Complete)) {
    ...
    public static String getaypVisitStatus(Visit lastVisit) {
    ...
    public static String getaypServiceVisitStatus(Visit lastVisit) {
        HashMap<String, Boolean> completionObject = new HashMap<>();
        try {
            JSONObject jsonObject = new JSONObject(lastVisit.getJson());
            JSONArray obs = jsonObject.getJSONArray("obs");
    ...
    public static String getaypProcedureVisitStatus(Visit lastVisit) {
        HashMap<String, Boolean> completionObject = new HashMap<>();
        try {
            JSONObject jsonObject = new JSONObject(lastVisit.getJson());
            JSONArray obs = jsonObject.getJSONArray("obs");
    ...
    public static void manualProcessVisit(Visit visit) throws Exception {
        ...
        VisitDetailsRepository visitDetailsRepository = AypLibrary.getInstance().visitDetailsRepository();
        VisitRepository visitRepository = AypLibrary.getInstance().visitRepository();
