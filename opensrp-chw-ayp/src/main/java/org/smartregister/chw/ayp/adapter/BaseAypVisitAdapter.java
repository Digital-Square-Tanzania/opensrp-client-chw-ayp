...
public class BaseAypVisitAdapter extends RecyclerView.Adapter<BaseAypVisitAdapter.MyViewHolder> {
    private Map<String, BaseAypVisitAction> aypVisitActionList;
    private Context context;
    private BaseAypVisitContract.View visitContractView;

    public BaseAypVisitAdapter(Context context, BaseAypVisitContract.View view, LinkedHashMap<String, BaseAypVisitAction> myDataset) {
        aypVisitActionList = myDataset;
    ...
    @NotNull
    @Override
    public BaseAypVisitAdapter.MyViewHolder onCreateViewHolder(@NotNull ViewGroup parent,
                                                                int viewType) {
        View v = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.ayp_visit_item, parent, false);
     ...
     * @return
     */
    private BaseAypVisitAction getByPosition(int position) {
        int count = -1;
        for (Map.Entry<String, BaseAypVisitAction> entry : aypVisitActionList.entrySet()) {
    ...
    public void onBindViewHolder(@NotNull MyViewHolder holder, int position) {
        ...
        BaseAypVisitAction aypVisitAction = getByPosition(position);
        if (aypVisitAction == null)

            return;

        if (!aypVisitAction.isEnabled()) {
        ...
        if (StringUtils.isNotBlank(aypVisitAction.getSubTitle())) {
            ...
            if (aypVisitAction.isEnabled()) {
                ...
                holder.descriptionText.setText(aypVisitAction.getSubTitle());

                boolean isOverdue = aypVisitAction.getScheduleStatus() == BaseAypVisitAction.ScheduleStatus.OVERDUE &&
                        aypVisitAction.isEnabled();
                ...
                holder.invalidText.setText(Html.fromHtml("<i>" + aypVisitAction.getDisabledMessage() + "</i>"));
            }
        } else {
            holder.descriptionText.setVisibility(View.GONE);
        }

        int color_res = getCircleColor(aypVisitAction);
        ...
        bindClickListener(holder.getView(), aypVisitAction);
    }

    private int getCircleColor(BaseAypVisitAction aypVisitAction) {

        int color_res;
        boolean valid = aypVisitAction.isValid() && aypVisitAction.isEnabled();
        if (!valid)
            return R.color.transparent_gray;

        switch (aypVisitAction.getActionStatus()) {
            case PENDING:
    ...
    private void bindClickListener(View view, final BaseAypVisitAction aypVisitAction) {
        if (!aypVisitAction.isEnabled() || !aypVisitAction.isValid()) {
            view.setOnClickListener(null);
            return;
        }

        view.setOnClickListener(v -> {
            if (StringUtils.isNotBlank(aypVisitAction.getFormName())) {
                visitContractView.startForm(aypVisitAction);
            } else {
                visitContractView.startFragment(aypVisitAction);
            }
