...
import org.smartregister.chw.ayp.adapter.BaseAypVisitAdapter;
...
public class BaseAypVisitActivity extends SecuredActivity implements BaseAypVisitContract.View, View.OnClickListener {

    private static final String TAG = BaseAypVisitActivity.class.getCanonicalName();
    protected Map<String, BaseAypVisitAction> actionList = new LinkedHashMap<>();
    protected BaseAypVisitContract.Presenter presenter;
    ...
    public static void startMe(Activity activity, String baseEntityID, Boolean isEditMode) {
        Intent intent = new Intent(activity, BaseAypVisitActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_base_ayp_visit_activity);
    ...
    public void setUpView() {
        ...
        mAdapter = new BaseAypVisitAdapter(this, this, (LinkedHashMap) actionList);
    ...
    protected void registerPresenter() {
        presenter = new BaseAypVisitPresenter(memberObject, this, new BaseAypVisitInteractor());
    }
    ...
    @Override
    public BaseAypVisitContract.Presenter presenter() {
        return presenter;
    }
    ...
    @Override
    public void startForm(BaseAypVisitAction aypHomeVisitAction) {
        current_action = aypHomeVisitAction.getTitle();

        if (StringUtils.isNotBlank(aypHomeVisitAction.getJsonPayload())) {
            try {
                JSONObject jsonObject = new JSONObject(aypHomeVisitAction.getJsonPayload());
                startFormActivity(jsonObject);
            } catch (Exception e) {
                Timber.e(e);
                String locationId = AypLibrary.getInstance().context().allSharedPreferences().getPreference(AllConstants.CURRENT_LOCATION_ID);
                presenter().startForm(aypHomeVisitAction.getFormName(), memberObject.getBaseEntityId(), locationId);
            }
        } else {
            String locationId = AypLibrary.getInstance().context().allSharedPreferences().getPreference(AllConstants.CURRENT_LOCATION_ID);
            presenter().startForm(aypHomeVisitAction.getFormName(), memberObject.getBaseEntityId(), locationId);
    ...
    @Override
    public void startFragment(BaseAypVisitAction aypHomeVisitAction) {
        current_action = aypHomeVisitAction.getTitle();

        if (aypHomeVisitAction.getDestinationFragment() != null)
            aypHomeVisitAction.getDestinationFragment().show(getSupportFragmentManager(), current_action);
    }
    ...
    @Override
    public Map<String, BaseAypVisitAction> getaypVisitActions() {
        return actionList;
    }
    ...
    @Override
    public BaseAypVisitContract.Presenter getPresenter() {
        return presenter;
    }
    ...
    public void onDialogOptionUpdated(String jsonString) {
        BaseAypVisitAction aypVisitAction = actionList.get(current_action);
        if (aypVisitAction != null) {
            aypVisitAction.setJsonPayload(jsonString);
        }
        ...
        if (requestCode == Constants.REQUEST_CODE_GET_JSON) {
            if (resultCode == Activity.RESULT_OK) {
                try {
                    ...
                    BaseAypVisitAction BaseAypVisitAction = actionList.get(current_action);
                    if (BaseAypVisitAction != null) {
                        BaseAypVisitAction.setJsonPayload(jsonString);
                    }
                } catch (Exception e) {
                    Timber.e(e);
                ...
                BaseAypVisitAction BaseAypVisitAction = actionList.get(current_action);
                if (BaseAypVisitAction != null)
                    BaseAypVisitAction.evaluateStatus();
            }
    ...
    public void onBackPressed() {
        displayExitDialog(BaseAypVisitActivity.this::finish);
    }
